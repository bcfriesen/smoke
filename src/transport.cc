...
void TRANSPORT::Propogate(PARTICLE &p, double dt)
{
  ...
  while (!stop)
  {
    ...
    tau_r = -1.0*log(1 - get_rng( use_cached_rng )); rng_count++;
...
void TRANSPORT::Compton_Scatter(PARTICLE &p)
{
  ...
  while (true)
  {
    ...
    double mu  = 1 - 2.0*get_rng( use_cached_rng ); rng_count++;
    double phi = 2.0*PI*get_rng( use_cached_rng ); rng_count++;
    ...
    if (p.type == gammaray)
    {
      ...
      double y = get_rng( use_cached_rng ); rng_count++;
...
void TRANSPORT::Emit_Particles(double dt)
{
  ...
  for (int k=0;k<n_x;k++)
  {
    ...
    if (get_rng( use_cached_rng ) < E[ind]*Epinv - n_add[ind]) n_add[ind]++; rng_count++;
    ...
    for (int q=n_particles;q<n_particles+n_add[ind];q++)
    {
      ...
      particle[q].x[0] = i*dx - x_cen + dx*get_rng( use_cached_rng ); rng_count++;
      particle[q].x[1] = j*dx - x_cen + dx*get_rng( use_cached_rng ); rng_count++;
      particle[q].x[2] = k*dx - x_cen + dx*get_rng( use_cached_rng ); rng_count++;

      // emit randomly over time step
      particle[q].t      = t_now + dt*get_rng( use_cached_rng ); rng_count++;
      ...
      particle[q].E_x   = rad.Sample_Ni56_Wavelength(particle[q].t);
